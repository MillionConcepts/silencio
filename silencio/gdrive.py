...
from silencio.utilz import separate


def is_drive_folder(drive_file):
...
class DriveBot(GoogleDrive):
    ...
    def find(self, pattern=None, root_path=None, verbose=False):
...
def match_drivefile_title(drivefile, pattern):
    if pattern is None:
        return True
    return re.match(pattern, drivefile["title"])
...
def process_match_group(fn, files, current_path, settings):
    if len(files) > 1:
        message = (
    ...
    files[0].GetContentFile(local)
    return local


def copy_gdrive(
    bot, drive_folder_id, patterns, settings, current_path=Path(".")
):
    contents = bot.ls(drive_folder_id)
    folders, files = separate(contents, is_drive_folder)
    ...
    for fn, grouper in groupby(matches, lambda m: m["title"]):
        process_match_group(fn, list(grouper), current_path, settings)
    for folder in folders:
        path = Path(current_path, folder["title"])
        if "folder" in patterns.keys():
            if not re.match(patterns["folder"], str(path)):
                continue
        copy_gdrive(bot, folder["id"], patterns, settings, path)
